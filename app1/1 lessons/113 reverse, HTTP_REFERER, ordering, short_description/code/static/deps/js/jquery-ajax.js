$(document).ready(function () {
    ...
    $("input[name='requires_delivery']").change(function() {
        ...
        // (xxx) xxx-xxxx formasında telefon nömrəsi qeydinin formatlanması.                    ORDERS / TEMPLATES / ORDERS / CREATE_ORDER.HTML   faylında İD-si `id_phone_number` olan elementi elde etdik
        document.getElementById('id_phone_number').addEventListener('input', function (e) {
            var x = e.target.value.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,4})/);
            e.target.value = !x[2] ? x[1] : '(' + x[1] + ') ' + x[2] + (x[3] ? '-' + x[3] : '');
        });
    
        // Müştəri tərəfində telefon nömrəsinin xxx-xxx-xx-xx şəklində düzgün olub olmadığını yoxlayırıq.  ORDERS / TEMPLATES / ORDERS / CREATE_ORDER.HTML   faylında İD-si `create_order_form` olan elementi elde etdik
